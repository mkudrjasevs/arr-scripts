...
DownloadFormat () {
	...
	if [ "$audioFormat" == "native" ]; then
		...
		tidal-dl-ng cfg quality_audio "$tidalQuality" 2>&1 | tee -a "/config/logs/$logFileName"
		deemixQuality=flac
		bitrateError=""
		audioFormatError=""
	fi
...
TidalClientTest () { 
	...
	while [ $i -lt 3 ]; do
		i=$(( $i + 1 ))
  		TidaldlStatusCheck
		tidal-dl-ng dl "https://tidal.com/browse/track/$tidalClientTestDownloadId" 2>&1 | tee -a "/config/logs/$logFileName"
...
DownloadProcess () {
	...
	do	
		...
		if [ "$2" == "TIDAL" ]; then
			...
			tidal-dl-ng dl "https://tidal.com/browse/album/$1" 2>&1 | tee -a "/config/logs/$logFileName"
...
ProcessWithBeets () {
	...
	beet -c /config/extended/beets-config.yaml -l /config/extended/beets-library.blb -d "$1" import -q --quiet-fallback=asis "$1" 2>&1 | tee -a "/config/logs/$logFileName"
...
DeemixClientSetup () {
	...
	# Create deemix config directory
